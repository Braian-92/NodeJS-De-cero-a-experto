...
export class GithubSha256Middleware {
  ...
  static verifyGithubSignature(
    ...
    GithubSha256Middleware.verifySignature(
      ...
      if(isValid) {
        ...
        return res.status(401).json({ json: 'Invalid signature' });
      }
    }).catch(error => {
      console.log("🚀 ~ GithubSha256Middleware ~ verifyGithubSignature ~ error:", error);
  ...
  private static async verifySignature(secret: string, header: string, payload: string) {
    try {
      ...
      console.log("🚀 ~ GithubSha256Middleware ~ verifySignature ~ error:", error)
      return false;
    }
  }
